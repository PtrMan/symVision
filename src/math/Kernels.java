...
public class Kernels
{
    ...
    public static class MarrHildrethOperator implements  IKernel<Float, Float>
    {
        ...
        public Float calculateAt(Vector2d<Float> position)
        {
            ...
            gaussianFactor = java.lang.Math.exp(-1.0f*(Math.squaredDistance(position.x, position.y)/(2.0*Math.power2(sigma))));
            scalingFactor = 1.0 - Math.squaredDistance(position.x, position.y)/(2.0*Math.power2(sigma));
